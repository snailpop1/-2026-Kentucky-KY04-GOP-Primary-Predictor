...
def test_model_outputs_are_valid_and_pre_results() -> None:
    ...
    assert "turnout_demographics" in output
    assert len(output["turnout_demographics"]) >= 2
    assert "endorsement_summary" in output
    assert len(output["endorsement_summary"]) >= 5
    assert "historical_vote_trend" in output
    assert len(output["historical_vote_trend"]) >= 6
    assert "turnout_environment_summary" in output
    assert len(output["turnout_environment_summary"]) >= 4
...
def test_processed_outputs_exist_and_have_last_updated() -> None:
    ...
    required = [
        ...
        "sensitivity.csv",
        "demographic_diagnostics.csv",
        "endorsement_summary.csv",
        "historical_vote_trend.csv",
        "historical_vote_trend_summary.csv",
        "turnout_registration_history.csv",
        "turnout_primary_history.csv",
        "turnout_environment_summary.csv",
...
def test_demographic_crosstabs_are_diagnostic_not_topline() -> None:
    main("2026-05-19")
    demographics = pd.read_csv(PROCESSED_DIR / "demographic_diagnostics.csv")
    age_summary = demographics[demographics["dimension"].eq("age_summary")]
    assert {"under_56", "56_plus"}.issubset(set(age_summary["group"]))
    younger = age_summary.loc[age_summary["group"].eq("under_56")].iloc[0]
    older = age_summary.loc[age_summary["group"].eq("56_plus")].iloc[0]
    assert younger["massie_pct"] > younger["gallrein_pct"]
    assert older["gallrein_pct"] > older["massie_pct"]
    assert older["electorate_share_pct"] > younger["electorate_share_pct"]
    assert demographics["model_role"].str.contains("diagnostic").all()


def test_endorsement_inventory_separates_modeled_and_audit_only_rows() -> None:
    main("2026-05-19")
    endorsements = pd.read_csv(PROCESSED_DIR / "endorsement_summary.csv")
    modeled = endorsements[endorsements["include_in_model"] == True]
    audit_only = endorsements[endorsements["include_in_model"] == False]
    assert {"Donald Trump", "Andy Barr", "Nate Morris", "Pete Hegseth", "Rand Paul"}.issubset(set(endorsements["endorser"]))
    assert "Lauren Boebert" in set(audit_only["endorser"])
    assert "Kentucky Right to Life" in set(audit_only["endorser"])
    assert "Northern Kentucky Right to Life" in set(audit_only["endorser"])
    assert "Nate Morris" in set(modeled["endorser"])


def test_historical_vote_trend_is_context_only() -> None:
    main("2026-05-19")
    history = pd.read_csv(PROCESSED_DIR / "historical_vote_trend.csv")
    summary = pd.read_csv(PROCESSED_DIR / "historical_vote_trend_summary.csv")
    primary = history[history["election_stage"].eq("primary")]
    assert {2012, 2020, 2022, 2024}.issubset(set(primary["year"]))
    recent_primary = primary[primary["year"].ge(2020)]
    assert recent_primary["massie_vote_share_pct"].between(75.0, 81.1).all()
    assert summary["notes"].str.contains("not rising|stable incumbent floor|not directly weighted", case=False, regex=True).any()


def test_turnout_environment_summary_matches_turnout_story() -> None:
    main("2026-05-19")
    registration = pd.read_csv(PROCESSED_DIR / "turnout_registration_history.csv")
    turnout = pd.read_csv(PROCESSED_DIR / "turnout_primary_history.csv")
    summary = pd.read_csv(PROCESSED_DIR / "turnout_environment_summary.csv")
    latest = registration.sort_values("cycle_year").iloc[-1]
    assert latest["cycle_year"] == 2026
    assert latest["rep_share_pct"] > 50.0
    assert turnout["gop_primary_turnout_rate_pct"].between(16.0, 29.0).all()
    beneficiary = summary.loc[summary["summary_group"].eq("beneficiary_call"), "notes"].iloc[0]
    assert "Gallrein-favorable" in beneficiary
